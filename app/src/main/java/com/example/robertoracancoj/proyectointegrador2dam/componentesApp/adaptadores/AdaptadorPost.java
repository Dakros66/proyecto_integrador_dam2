...
import com.example.robertoracancoj.proyectointegrador2dam.componentesApp.utils.PreferencesUtil;
...
public class AdaptadorPost extends BasePostAdapter{

    public static final String TAG = AdaptadorPost.class.getSimpleName();
    ...
    public AdaptadorPost(final MainActivity activity, SwipeRefreshLayout swipeContainer) {
        super(activity);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TipoItem.ITEM.getTypeCode()) {
            return new PostViewHolder(inflater.inflate(R.layout.celda_post_item_list, parent, false),
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (position >= getItemCount() - 1 && isMoreDataAvailable && !isLoading) {
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    ...
                    if (activity.hasInternetConnection()) {
                        isLoading = true;
                        postList.add(new Post(TipoItem.LOAD));
        ...
        if (getItemViewType(position) != TipoItem.LOAD.getTypeCode()) {
    ...
    private void hideProgress() {
        if (!postList.isEmpty() && getItemViewType(postList.size() - 1) == TipoItem.LOAD.getTypeCode()) {
